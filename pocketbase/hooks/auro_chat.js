routerAdd(
  ...
  (e) => {
    try {
      ...
      const conv = $ai.agent('auro-assistente').getOrCreateConversation({
        user_id: userId,
        id: body.conversation_id || null,
      })

      const iter = $ai.agent('auro-assistente').chat({
        user_id: userId,
...
routerAdd(
  'GET',
  '/backend/v1/auro/chats/{conversationId}/messages',
  (e) => {
    try {
      const userId = e.auth?.id
      if (!userId) return e.unauthorizedError('auth required')
      return e.json(
        200,
        $ai.agent('auro-assistente').listMessages({
