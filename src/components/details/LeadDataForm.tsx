...
export interface LeadDataFormRef {
  isDirty: boolean
  saveChanges: () => Promise<boolean>
}

export const LeadDataForm = forwardRef<LeadDataFormRef, { lead: Lead; onExit?: () => void }>(
  ({ lead, onExit }, ref) => {
    ...
    useEffect(() => {
      const dirty =
        data.nome !== (lead.nome || '') ||
        data.tipo !== lead.tipo ||
        data.segmento !== lead.segmento ||
        data.status !== lead.status ||
        data.prioridade !== lead.prioridade ||
        data.origem !== lead.origem ||
        data.observacoes !== (lead.observacoes || '') ||
        JSON.stringify(data.tags || []) !== JSON.stringify(lead.tags || [])
      setIsDirty(dirty)
    }, [data, lead])
